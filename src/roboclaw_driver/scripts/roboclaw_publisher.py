...
class RoboclawPublisher:
    ...
    def read_halls(self):
        ...
        # pending roboclaw limit switch functionality
        return gpio.input(self.HALLPIN_1), gpio.input(self.HALLPIN_2)


    def talker(self):
        """proper ROS publisher node, reads data from roboclaw and publishes to "nuada_roboclaw_cmd" topic"""
        cmd_pub = rospy.Publisher('nuada_roboclaw_cmd', armCmd, queue_size=10)
        rospy.init_node('roboclaw_talker', anonymous=True)
        ...
        while not rospy.is_shutdown():
            ...
            base_pos = self.rads_enc_cnts_conv(cnts_per_rev=self.joint_cnts_per_rev[self.BASE_IDX], enc_counts=self.rc.ReadEncM2(self.joint_addresses[self.BASE_IDX]))
            elbow_pos = self.rads_enc_cnts_conv(cnts_per_rev=self.joint_cnts_per_rev[self.ELBOW_IDX], enc_counts=self.rc.ReadEncM1(self.joint_addresses[self.ELBOW_IDX]))
            wrist_pos = self.rads_enc_cnts_conv(cnts_per_rev=self.joint_cnts_per_rev[self.WRIST_IDX], enc_counts=self.rc.ReadEncM2(self.joint_addresses[self.WRIST_IDX]))
            claw_pos = self.rads_enc_cnts_conv(cnts_per_rev=self.joint_cnts_per_rev[self.CLAW_IDX], enc_counts=self.rc.ReadEncM1(self.joint_addresses[self.CLAW_IDX]))
            ...
            msg.hall_vals[0], msg.hall_vals[1] = self.read_halls()
